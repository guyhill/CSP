...

{
    ...
    if (card == 0) {
        std::cout << "ERROR: no fractional variables (FRAC=" << FRAC
                  << std::endl;
        write_sol(fsolution);
        CSPexit(EXIT_ERROR); // exit(1);
    }
    ...
    case BETTERLP:
        ...
        if (stack == NULL) {
            std::cout << "ERROR: not memory for 'frac' in branching"
                      << std::endl;
            CSPexit(EXIT_ERROR); // exit(1);
        }
        card = 0;
...
int con_branching() {
    ...
    if (upperb < ceil(lowerb - ZERO) + ZERO) {
        return (1);
    }
...
static void constraint_branching(int nvar, VARIABLE **xvar, CONSTRAINT **con0,
                                 CONSTRAINT **con1)
