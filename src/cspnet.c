...

{
    ...
    while (l--) {
        pro = prot_level + l;
        if (!sleep[l]) {
            if (protection_level(pro->sen->var, pro->sense, NULL, NULL, NULL,
                                 primal, 'C') <
            ...
            while (k--) {
                pro0 = prot_level + k;
                if (pro0->sense * primal[pro0->sen->var->index] >
...
// type   C = real numbers  ;   I = integer numbers
{
    ...
    if (Nlp == NULL) {
        std::cout << "ERROR: There is not enough memory for the first LP"
                  << std::endl;
...
double protection_level(VARIABLE *var, int goal, int *nvar, VARIABLE **lvar,
                        double *cvar, double *primal, char type)
// type 'C' = real numbers  ;  'I' = integer numbers
{
    ...
    if (num == -1) {
        std::cout << "ERROR: variable " << var->name << " not in the network"
                  << std::endl;
        CSPexit(EXIT_ERROR); // exit(1);
    }
    ...
    if (type == 'I') {
        ...
        if (JJmipopt(Nlp)) {
            std::cout << " it was not possible to solve with mip-opt "
                      << std::endl;
            JJlpwrite(Nlp, netlpname);
            CSPexit(EXIT_LPSOLVER); // exit(1);
        }
    ...
    if (nvar) {
        ...
        for (k = 0; k < Rncells; k++) {
            ...
            // if (cost[k]>ZERO) printf(" ind %d uval %lf
            // ",k,columns[k].uvalue); if (-cost[k]>ZERO) printf(" ind %d lval
            // %lf ",k,columns[k].lvalue);
        }
        *nvar = card;
        free(cost);
        cost = NULL; /*PWOF*/
    ...
    if (primal) {
        ...
        if (JJgetx(Nlp, x, 0, mac - 1)) { // x contains the solution values
                                          // ​​of the variables listed
            std::cout << "ERROR: no solution is avalaible " << std::endl;
            CSPexit(EXIT_LPSOLVER); // exit(1);
        }
        for (k = 0; k < mac; k++) {
            primal[net2cell[k]] = x[k];
        }
        free(x);
        x = NULL; /*PWOF*/
    }
