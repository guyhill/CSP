...

{
    ...
    while (l--) {
        ...
        if (!sleep[l]) {
            ...
            if (index != -1) {
                free_col(index, bd);
                if (protection_level(pro->sen->var, pro->sense, &nvar, xvar,
                                     cvar, NULL, 'C') <
                    pro->sense * pro->sen->var->nominal + pro->level - ZERO) {
                    con = capacity_constraint(nvar, xvar, cvar, pro->sen->var,
                                              pro->level);
...
CONSTRAINT *capacity_constraint(int nvar, VARIABLE **xvar, double *cvar,
                                VARIABLE *var, double rhs)

{
    ...
    if (x == NULL) {
        std::cout << " ERROR: not enough memory for the X (cspcapa.c)"
                  << std::endl;
        CSPexit(EXIT_MEMO); // exit(1);
    }
