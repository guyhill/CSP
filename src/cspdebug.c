...

{
    ...
    for (k = 0; k < nsensitive; k++) {
        ...
        fprintf(fich, " %f %f %f\n", sensitive[k].lpl, sensitive[k].upl,
                col->nominal);
    }
    fprintf(fich, "%d\n", nsupport - nsensitive);
...

{
    ...
    fclose(fich);
    if (weight != upperb) {
        std::cout << "ERROR:  weight=" << weight << "   upperb=" << upperb
                  << std::endl;
...
int control_ind() {
    ...
    for (i = 0; i < ncols; i++) {
        ...
        case WAITING:
            if (val > ZERO) {
                std::cout << "ERROR 1c: var=" << i << " with stat=" << stat
                          << " val=" << val << std::endl;
                return (1);
            }
            break;
        case FIX_LB:
            if (val > ZERO) {
                std::cout << "ERROR 1b: var=" << i << " with stat=" << stat
                          << " val=" << val << std::endl;
                return (1);
            }
            break;
        case FIX_UB:
            if (val < 1 - ZERO) {
                std::cout << "ERROR 1d: var=" << i << " with stat=" << stat
                          << " val=" << val << std::endl;
                return (1);
            }
            break;
        ...
        case LP_BA:
            if (lpt < 0 || lpt >= mac) {
                std::cout << "ERROR 2: var=" << i << " with stat=" << stat
                          << " lp=" << lpt << std::endl;
                return (1);
            }
            if (cind[lpt] != columns + i) {
                std::cout << "ERROR 3: var=" << i << " with lp=" << lpt
                          << std::endl;
                return (1);
            }
            break;
        default:
            std::cout << "ERROR 4: " << i << " with unknown stat=" << stat
                      << std::endl;
            return (1);
        }
    }
    for (i = 0; i < nrows; i++) {
        ...
        case FIX_UB:
            if (lpt != -1) {
                std::cout << "ERROR 5: row=" << i << " with stat=" << stat
                          << " lp=" << lpt << std::endl;
                return (1);
            }
            break;
        ...
        case LP_BA:
            if (lpt < 0 || lpt >= mar) {
                std::cout << "ERROR 6: row=" << i << " with stat=" << stat
                          << " lp=" << lpt << std::endl;
                return (1);
            }
            if (rind[lpt] != rows[i]) {
                std::cout << "ERROR 7: row=" << i << " with lp=" << lpt
                          << std::endl;
                return (1);
            }
            break;
        default:
            std::cout << "ERROR 8: " << i << " with unknown stat=" << stat
                      << std::endl;
            return (1);
        }
    }
    ...
    for (i = 1; i < mac; i++) {
        if (i != cind[i]->lp) {
            std::cout << "ERROR 5: col i=" << i << "  lp=" << cind[i]->lp
                      << std::endl;
            return (1);
        }
    }
    for (i = 1; i < mar; i++) {
        if (i != rind[i]->lp) {
            std::cout << "ERROR 6: row i=" << i << "  lp=" << rind[i]->lp
                      << std::endl;
            return (1);
        }
    }
...

{
    ...
    std::cout << " Row type=" << con->type << "  lp=" << con->lp
              << "  vio=" << violated(con) << std::endl;
    ...
    case BRIDGE:
        ...
        break;
    default:
        std::cout << "ERROR print: unknown type " << con->type
                  << " of constraint in paint" << std::endl;
        CSPexit(EXIT_ERROR); // exit(1);
    }
...

{
    ...
    case CAPACITY:
        ...
        break;
    default:
        std::cout << "ERROR d: unknown type " << con->type << " of constraint"
                  << std::endl;
        CSPexit(EXIT_ERROR); // exit(1);
    }
    ...
    case CAPACITY:
        break;
    default:
        std::cout << "ERROR e: unknown type " << con->type << " of constraint"
                  << std::endl;
        CSPexit(EXIT_ERROR); // exit(1);
    }
...

{
    ...
    case COVER:
        ...
        break;
    default:
        std::cout << "ERROR f: unknown type " << con->type << " of constraint"
                  << std::endl;
        CSPexit(EXIT_ERROR); // exit(1);
    }
    ...
    for (i = 0; i < ncols; i++) {
        ...
        if (fabs(c - value[i]) > ZERO) {
            std::cout << "ERROR: val=" << i << "  old_coef=" << value[i]
                      << "  new_coef=" << c << std::endl;
            CSPexit(EXIT_ERROR); // exit(1);
        }
    }
    ...
    case GOMORY:
        break;
    default:
        std::cout << "ERROR 8: unknown type " << con->type << " of constraint"
                  << std::endl;
        CSPexit(EXIT_ERROR); // exit(1);
    }
    free(value);
...

{
    ...
    while (ncut--) {
        ...
        if (val < minvalue - ZERO || val > minvalue + ZERO) {
            std::cout << " ERROR: in control cut: read=" << val
                      << " virtual=" << minvalue << std::endl;
            CSPexit(EXIT_ERROR); // exit(1);
        }
        cutlst += nnod;
    }
...

{
    ...
    if ((val < -ZERO && con->sense == 'G') ||
        (val > ZERO && con->sense == 'L')) {
        std::cout << "\n\nWARNING: violated constraint<<<<<<<<<<<<<<<<<<<<"
                  << std::endl;
    }
    for (j = 0; j < 9; j++) {
