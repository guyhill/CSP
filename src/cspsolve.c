...
int load_lp() {
    ...
    for (k = 0; k < ncols; k++) {
        ...
        default:
            col->lp = mac;
            if (mac == MAX_COLS_LP) {
                std::cout << "ERROR: mac=" << mac << " : too many LP variables"
                          << std::endl;
                CSPexit(EXIT_ERROR); // exit(1);
            }
            cind[mac] = col;
    ...
    lp = JJloadprob(probname, mac, mar, 0, 1, objx, rhsx, senx, matbeg, matcnt,
                    matind, matval, bdl, bdu, NULL, NULL, NULL, NULL, NULL,
                    NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
                    NULL, NULL, NULL, macsz, marsz, matsz, 0, 0, 0, 0, 0);
    if (lp == NULL) {
        std::cout << "ERROR: loading an LP (probably because no hardware-key)"
                  << std::endl;
...

{
    ...
    if ((rmatbeg = (int *)malloc(sizeof(int) * rcnt)) == NULL) {
        std::cout << "There is not enough memory for vector RMATBEG"
                  << std::endl;
        CSPexit(EXIT_MEMO); // exit(1);
    }
    if ((rmatind = (int *)malloc(largemem * sizeof(int))) == NULL) {
        std::cout << "There is not enough memory for vector RMATIND"
                  << std::endl;
        CSPexit(EXIT_MEMO); // exit(1);
    }
    if ((rmatval = (double *)malloc(largemem * sizeof(double))) == NULL) {
        ...
        if ((rmatval = (double *)malloc(largemem * sizeof(double))) == NULL) {
            std::cout
                << "There is not enough memory for vector RMATVAL (addrow)"
                << std::endl;
            CSPexit(EXIT_MEMO); // exit(1);
        }
    }
    ...
    for (i = 0; i < rcnt; i++) {
        rmatbeg[i] = nzcnt;
        nzcnt += add_row(queue[i], rhs + i, sense + i, rmatind + nzcnt,
                         rmatval + nzcnt);
        if (nzcnt > largemem) {
            std::cout << "ERROR: mem for add rows = " << largemem
                      << " not enought" << std::endl;
            CSPexit(EXIT_MEMO); // exit(1);
        }
    }

    if (JJaddrows(lp, ccnt, rcnt, nzcnt, rhs, sense, rmatbeg, rmatind, rmatval,
                  NULL, NULL)) {
        std::cout << " ERROR: it was not possible to add constraints"
                  << std::endl;
        CSPexit(EXIT_LPSOLVER); // exit(1);
    }
...
static int add_row(CONSTRAINT *con, double *rhs, char *sense, int *rmatind,
                   double *rmatval)

{
    ...
    if (con->lp != -1) {
        std::cout << "ERROR: constraints already in the LP (" << con->lp << ")"
                  << std::endl;
        CSPexit(EXIT_ERROR); // exit(1);
    }
    con->lp = mar;
    ...
    *rhs = con->rhs;
    ...
    case COVER:
        ...
        break;
    default:
        std::cout << "ERROR: unknown type " << con->type << " of constraint"
                  << std::endl;
        CSPexit(EXIT_ERROR); // exit(1);
    }
...

{
    ...
    if (JJaddcols(lp, ccnt, nzcnt, cobj, cmatbeg, cmatind, cmatval, cbdl, cbdu,
                  NULL)) {
        CSPexit(EXIT_LPSOLVER);
        // exit(1);
    }
...

{
    ...
    anterior = (struct PRICE *)malloc(
        sizeof(struct PRICE)); // PWOF added memory allocation
...

{
    ...
    PRIC:
        while (pricing(cleaning)) {
            ...
            t1 = seconds();
            if (JJoptimize(lp)) {
                std::cout << " it was not possible to solve with OPTIMIZE "
                          << std::endl;
                CSPexit(EXIT_LPSOLVER); // exit(1);
            }
            toptimize += seconds() - t1;
    ...
    for (i = 1; i < mac; i++) {
        ...
        if (rc > ZERO) {
            if (ceil(lowerb + rc - ZERO) + ZERO > upperb) {
                status[i] = FIX_LB;
                num0++;
            } else if (list_pricing &&
                       ceil(lowerb + 2 * rc - ZERO) + ZERO > upperb) {
                status[i] = WAITING;
...

{
    ...
    for (i = 1; i < mac; i++) {
        if (status[i] == FIX_UB) {
            JJgetcols(lp, &nzcnt, cmatbeg, cmatind, cmatval, mar, &surplus, i,
                      i);
            for (j = 0; j < nzcnt; j++) {
...

{
    ...
    if (col->lp < 0) {
        std::cout << "ERROR: variable not in the LP: stat=" << col->stat
                  << std::endl;
        CSPexit(EXIT_ERROR); // exit(1);
    }
    ...
    if (JJchgbds(lp, 1, index, lu, value)) {
        std::cout << "ERROR: it is not possible to fix LP-var " << col->index
                  << std::endl;
        CSPexit(EXIT_LPSOLVER); // exit(1);
    }
...

{
    ...
    if (rmatind == NULL) {
        std::cout << " ERROR: not enough memory for branch-constraint"
                  << std::endl;
        CSPexit(EXIT_MEMO); // exit(1);
    }
    for (i = 0; i < cont; i++) {
        rmatind[i] = row->stack[i]->lp;
    }
    ...
    if (JJaddrows(lp, 0, 1, cont, &rhs, &sense, &rmatbeg, rmatind, rmatval,
                  NULL, NULL)) {
        std::cout << " ERROR: it was not possible to add branch constraint"
                  << std::endl;
        CSPexit(EXIT_LPSOLVER); // exit(1);
    }
    free((void *)rmatval);
    ...
    if (JJdelrows(lp, mar, mar)) {
        std::cout << " ERROR: it was not possible to delete branch constraint"
                  << std::endl;
        CSPexit(EXIT_LPSOLVER); // exit(1);
    }
    return (lb);
...
int integer_solution(void) {
    ...
    for (nsup = 0; nsup < nfixed1; nsup++) {
        sup[nsup] = fixed1[nsup];
    }
    for (i = 1; i < mac; i++) {
        if (xval[i] > ZERO) {
            sup[nsup++] = cind[i];
...
void setup_lp() { JJoptimize(lp); }

void control() {
    int i, j;

    if (mar != JJgetmar(lp) || mac != JJgetmac(lp)) {
        std::cout << "ERROR: mar=" << mar << " mac=" << mac
                  << " getmar=" << JJgetmar(lp) << " getmac=" << JJgetmac(lp)
                  << std::endl;
        CSPexit(EXIT_ERROR); // exit(1);
    }
    for (i = 0; i < ncols; i++) {
        ...
        if (j > 0 && cind[j]->index != i) {
            std::cout << "\n ERROR in control: cind[" << j
                      << "]=" << cind[j]->index << " stat[" << i
                      << "]=" << columns[i].lp << std::endl;
            CSPexit(EXIT_ERROR);
        }
    }
}

void activa_pricing() { pricing_util = 0; }
