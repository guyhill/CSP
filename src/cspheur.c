...

{
    ...
    for (k = 0; k < nprot_level; k++) {
        pro = prot_level + k;
        obj = protection_2level(pro->sen->var, pro->sense, pro->level,
                                1); // k==2 in problem subtable
    ...
    if (sup == NULL) {
        std::cout << "There is not enough memory for SUP in heuristic"
                  << std::endl;
        CSPexit(EXIT_MEMO); // exit(1);
    }
    weight = 0;
    ...
    for (l = 0; l < nstack; l++) {
        ...
        if (k < nrows) {
            ...
            for (k = 0; k < nprot_level; k++) {
                pro = prot_level + k;
                if (protection_2level(pro->sen->var, pro->sense, pro->level,
                                      0) > ZERO) {
                    break;
                }
            }
...

{
    ...
    for (i = 0; i < Rncells; i++) {
        ...
        if (status[i]) {
            ...
            cell2net[i] = mac / 2;
            Nobjx[mac] =
                (var->val > EPSILON - ZERO ? 0.0
                                           : (1 - var->val) * var->weight + 1);
            Nmatbeg[mac] = l;
            Nmatcnt[mac] = 0;
    ...
    Nlp = JJloadprob(Nprobname, mac, mar, 0, 1, Nobjx, Nrhsx, Nsenx, Nmatbeg,
                     Nmatcnt, Nmatind, Nmatval, Nbdl, Nbdu, NULL, NULL, NULL,
                     NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
                     NULL, NULL, NULL, NULL, NULL, macsz, marsz, matsz, 0, 0, 0,
                     0, 0);

    if (Nlp == NULL) {
        std::cout << "ERROR: There is not enought memory for the first LP"
                  << std::endl;
...

{
    ...
    lu = 'L'; // Modified on september 2001, to allow protection leves >
              // external bounds
    bd = goal;
    ...
    if (JJoptimize(Nlp)) {
        ...
        std::cout << " CELL=" << var->name << "  type=" << type
                  << "  prot= " << goal << " (lpstat=" << netstatus;
        if (netstatus == JJ_INForUNB || netstatus == JJ_INFEASIBLE ||
            netstatus == JJ_UNBOUNDED) {
            opt = INF;
            goto FUERA;
        }

        std::cout << " it was not possible to solve with -optimize- (lpstat="
                  << netstatus << ")";
    ...
    if (netstatus != JJ_OPTIMAL && netstatus != 5 /*11*/ &&
        lpiter != MAXLPITER) { // changed by Salome 08/02/12
        opt = BIGVALUE;
    } else {
        JJgetobjval(Nlp, &opt);
        ...
        if (mode) {
            ...
            if (x == NULL) {
                std::cout << "There is not enough memory for RED COST"
                          << std::endl;
                CSPexit(EXIT_MEMO); // exit(1);
            }
...

{
    ...
    for (l = 0; l < nrows; l++) {
        if (rows[l]->type == CAPACITY) {
            ...
            stack = rows[l]->stack;
            while (card--) {
                if (stack[card]->val > 1 - ZERO &&
                    stack[card]->sensitive == 0) {
                    rnumber[l]++;
                    rvar[l] = stack[card];
                }
            }
    ...
    *nlist = 0;
    for (l = 0; l < Rncells; l++) {
        if (columns[l].val > 1 - ZERO && columns[l].sensitive == 0 &&
            status[l] == 0) {
            list[(*nlist)++] = l;
        }
    }
