...
void CSPSetDoubleConstant(const int VarNumber, double VarValue) {
    PPCSPSetDoubleConstant(VarNumber, VarValue);
}
double CSPGetDoubleConstant(const int VarNumber) {
    return PPCSPGetDoubleConstant(VarNumber);
}
void CSPSetIntegerConstant(const int VarNumer, int VarValue) {
    PPCSPSetIntegerConstant(VarNumer, VarValue);
}
int CSPGetIntegerConstant(const int VarNumber) {
    return PPCSPGetIntegerConstant(VarNumber);
}
int CSPoptimize(IProgressListener *ProgressListener) {
    return PPCSPoptimize(ProgressListener);
}
int CSPloadprob(int nsums_, double *rhs_, int ncells_, double *data_,
    ...
    return PPCSPloadprob(nsums_, rhs_, ncells_, data_, weight_, status_, lpl_,
                         upl_, lb_, ub_, names_, nlist_, listcell_, listcoef_);
}
int CSPfreeprob() { return PPCSPfreeprob(); }
int CSPsolution(int *lowerb_, int *upperb_, char *status_) {
    return PPCSPsolution(lowerb_, upperb_, status_);
}
int CSPrelbounds(int nlist, int *list, double *ub, double *lb, char type) {
    return PPCSPrelbounds(nlist, list, ub, lb, type);
}
} // namespace SCIPv
...
int PPCSPoptimize(IProgressListener *ProgressListener) {
    ...
    for (k = 0; k < nsensitive; k++) {
        if (sensitive[k].lpl > ZERO) {
            initpl++;
        }
        if (sensitive[k].upl > ZERO) {
            initpl++;
        }
    }
    ...
    for (k = 0; k < Rncells; k++) {
        if (columns[k].val < ZERO) {
            columns[k].val = 2 * ZERO;
        }
    }

    if (!upperb) {
        upperb = INF;
        if (Rncells < 1000 && Rnsums < 1000) {
            heuristic(1);
        } else {
            heuristic(0);
        }
    ...
    for (k = 0; k < nsensitive; k++) {
        sensitive[k].var->val = 1.0;
    }

    upperb_root = upperb_init = upperb;
    ncols = Rncells;
    cpool = ccapa = cbend = cbrid = ccove = ccove2 = cmatc = cgomo = cpath =
        cpath2 = 0;
    rowsinit = nrows;
    ...
    while (read_prob()) {
        ...
        if (ProgressListener != NULL) {
            ...
            ProgressListener->UpdateDiscrepancy(100 * (upperb - lowerb) /
                                                lowerb); // percentages
        ...
        JUMP:
            ...
            if (t1 > MAX_TIME) {
                ...
                if (ProgressListener !=
                    NULL) // Optimal approach (in case Modular approach,
                          // ProgressListener==NULL)
                {
                    ...
                    tstart = tstart +
                             (seconds() -
                              ttmp); // exclude time spent to ask for more time
                }

                if (res > 0) {
                    t0 = seconds();
                    MAX_TIME = res * 60.0; // res in minutes (integer), MAX_TIME
                                           // in seconds (double))
                } else {
                    unload_lp();
                    goto OUT;
        ...
        if (root) {
            ...
            lowerb_root = lowerb;
            upperb_root = upperb;
            if (lowerb_root > upperb_root) {
                lowerb_root = upperb_root;
            }
            root = 0;
        }
        unload_lp();
...
OUT:
    ...
    if (fich == NULL) {
        ...
        fprintf(fich,
                "cells sums cells sums |P| pl0 l1u1 l1u0 l0u1 l0u0  tprep  "
                "theur0   UB0  theur1  theur*    UB1     LB1     time   opt   "
                "SUP   time  BestLB  nodes    Ttime   Tsepar  Tprice   Toptim  "
                "maxmar cuts  pool  bender capa  bridge cover cover2 gomo comb "
                " path path2 iterations  UBtype\n\n");
    }
    fclose(fich);
    ...
    fprintf(
        fich,
        "%3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %8.1f %8.1f %8d %8.1f %8.1f "
        "%8d %8.1f %8.1f %8d %4d %8.1f %5d %4d %8.2f %8.1f %8.1f %8.1f %5d %5d "
        "%5d %5d %5d %5d %5d %5d %5d %5d %5d %5d   %5d    %c\n",
        CSPcells, CSPnsums, NEWcells, NEWnsums, nsensitive, initpl, l1u1, l1u0,
        l0u1, l0u0, tprep, theur0, upperb_init, theur1, theur, upperb_root,
        (float)ceil((double)lowerb_root - ZERO), troot, upperb,
        nbetter - nsensitive, topti, (int)bestLB, nbr, cpu, tseparation,
        tpricing, toptimize, lprows, nrows, cpool, rowsinit, ccapa, cbrid,
        ccove, ccove2, cgomo, cmatc, cpath, cpath2, iterations, ubtype);
    fclose(fich);
...

{
    ...
    filestr.open(
        fCSPlog); /*PWOF to write logfile in temp-directory 03-02-2014*/
    ...
    for (k = 0; k < Rncells; k++) {
        ...
        case 9:
            ...
            break;
        default:
            std::cout << "ERROR: '" << col->sensitive
                      << "' unknown type of cell" << std::endl;
            return (1);
        }
    }
    ...
    prot_level = (PROT_LEVEL *)malloc(
        2 * nsensitive * sizeof(PROT_LEVEL)); // nivel de proteccion
    if (sensitive == NULL && prot_level == NULL) {
        return (1);
    }
...
static void init_variable_type() {
    ...
    for (i = 0; i < Rncells; i++) {
        if (columns[i].stat == LP_LB) {
            columns[i].stat = WAITING;
        }
    }

    stack = (VARIABLE **)malloc(Rncells * sizeof(VARIABLE *));
    if (stack == NULL) {
        std::cout << "ERROR: not enough memory for INIT VARIABLE TYPe"
                  << std::endl;
        CSPexit(EXIT_ERROR); // exit(1);
    }
...
// type 'C' = real numbers ; 'I' = integer numbers
{
    ...
    for (k = 0; k < nlist; k++) {
        ...
        if (l != -1) {
            ub[k] =
                protection_level(columns + l, 1, NULL, NULL, NULL, NULL, type);
            lb[k] = -protection_level(columns + l, -1, NULL, NULL, NULL, NULL,
                                      type);
        }
    }
    unload_network();
...
int CSPpartialbounds() {
    ...
    for (k = 0; k < Rncells; k++) {
        if (status[k] > ZERO) {
            ...
            lb =
                -protection_level(columns + k, -1, NULL, NULL, NULL, NULL, 'C');
            std::cout << " " << columns[k].name << " : " << lb << "  "
                      << columns[k].nominal << "  " << ub << std::endl;
        }
    }
    unload_network();
    free(status);
...

{
    ...
    if (file == NULL) {
        std::cout << "ERROR: not possible to open file " << filename
                  << std::endl;
        return 1;
    }
    fprintf(file, "0\n %d\n", Rncells);
    for (i = 0; i < Rncells; i++) {
        ...
        fprintf(file, "%10.1f %10.1f ", (col->nominal) - (col->lvalue),
                (col->nominal) + (col->uvalue));
        if (col->sensitive) {
...
int CSPtestprob(int nsums_, double *rhs_, int ncells_, double *data_,
                int *weight_, char *status_, double *lpl_, double *upl_,
                double *lb_, double *ub_, char **names_, int *nlist_,
                int *listcell_, signed char *listcoef_)

{
    ...
    for (s = k = 0; k < nsums_; k++) {
        ...
        if (fabs(val) > ZERO) {
            std::cout << "ERROR: nominal constraint=" << k
                      << " with difference " << val << std::endl;
            return 1;
        }
    }
    for (k = 0; k < ncells_; k++) {
        if (abs(weight_[k]) > INF / 10) {
            std::cout << "ERROR: weight of " << k << " = " << weight_[k]
                      << std::endl;
            return 1;
        }
        if (data_[k] + ZERO < lb_[k] || data_[k] - ZERO > ub_[k]) {
            std::cout << "ERROR: data of " << k << " = " << lb_[k] << " < "
                      << data_[k] << " < " << ub_[k] << std::endl;
            return 1;
        }
        if (status_[k] != 's' && status_[k] != 'z' && status_[k] != 'u') {
            std::cout << "ERROR: status of " << k << " = " << status_[k]
                      << std::endl;
            return 1;
        }
    }
