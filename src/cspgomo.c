...
static int chvatal(int, double *, CONSTRAINT **, int *, CONSTRAINT **, int, int,
                   int *, double, int);
...

{
    ...
    if (mar != JJgetmar(lp)) {
        std::cout << "ERROR: mar=" << mar << "  getmar(lp)=" << JJgetmar(lp)
                  << std::endl;
    ...
    for (i = 1; i < mar; i++) {
        ...
        if (viola > MIN_VIOLA + ZERO) {
            ...
            if (k && chvatal(k, rb, ri, card, stack, GOMORY, MAX_CUTS_GOMO,
                             cstat, viola, i) < 0) {
                break;
            }
        }
...
static int chvatal(int k, double *rb, CONSTRAINT **con, int *card,
                   CONSTRAINT **stack, int type, int max_local_cuts, int *cstat,
                   double viola, int number)

{
    ...
    if (row->stack == NULL) {
        std::cout << "There is not enough memory for ROW->stack " << v
                  << std::endl;
        CSPexit(EXIT_MEMO); // exit(1);
    }
    ...
    if (new_row(row)) {
        ...
        if ((*card) == max_local_cuts || (*card) == MAX_CUTS_ITER) {
            std::cout << "WARNING: Too much local rows (" << max_local_cuts
                      << ")" << std::endl;
            return (-1);
        }
    }
...
static int different_gomory(int v, VARIABLE **ri, int *rc, int card,
                            CONSTRAINT **stack)

{
    ...
    while (card--) {
        if ((l = stack[card]->card) == v) {
            ...
            while (l-- && ri[l] == ptr_var[l] &&
                   fabs(rc[l] - ptr_coe[l]) < ZERO) {
                ;
            }
            if (l < 0) {
                return (1);
...

{
    ...
    if (rb > ZERO) {
        while (card--) {
            ra[stack[card]->index] += coef[card] * rb;
        }
        *rhs += con->rhs * rb;
    }
...

{
    ...
    for (j = 0; j < mac; j++) {
        if (fabs(z[j] - ra[j]) > ZERO) {
            std::cout << "ERROR in Gomory: row=" << i << " col=" << j
                      << " z=" << z[j] << " ra=" << ra[j] << std::endl;
        }
    }
